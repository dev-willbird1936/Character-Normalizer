# Changelog

## 0.1.2

- Replaced the synthetic reference-sheet demo with a real Pexels stock human source photo.
- Added normalized photoreal front, side, and body outputs generated from that same person.
- Removed the older demo source files from documentation assets.

## 0.1.1

- Replaced the before/after reference sheet with a synthetic public-safe visual using one coherent identity across source, front, side, and body views.
- Reduced visible watermark noise while keeping project ownership marks in the visual and PNG metadata.

## 0.1.0

- Added browser UI for front, modify, and side/body generation flows.
- Added Electron desktop popup runner.
- Added streaming generation results so each asset appears when ready.
- Added source identity and body-shape preservation prompt rules.
- Added Codex CLI, raw image CLI, and mock generation providers.
- Added public-safe demo assets and GitHub documentation.
